...
func (it *TransationRMSession) Query(sqlorArgs string) ([]map[string][]byte, error) {
	...
	var error = it.Client.Call(TransactionReqDTO{MapperName: it.MapperName, Status: it.Status, TransactionId: it.TransactionId, Sql: sqlorArgs, ActionType: ActionType_Query, OwnerId: it.OwnerId}, &result)
...
func (it *TransationRMSession) Exec(sqlorArgs string) (*Result, error) {
	var result TransactionRspDTO
	var error = it.Client.Call(TransactionReqDTO{MapperName: it.MapperName, Status: it.Status, TransactionId: it.TransactionId, Sql: sqlorArgs, ActionType: ActionType_Exec, OwnerId: it.OwnerId}, &result)
	if error == nil && result.Error != "" {
		error = utils.NewError("TransationRMSession", result.Error)
	}
...
func (it *TransationRMSession) Rollback() error {
	...
	var result TransactionRspDTO
	return it.Client.Call(TransactionReqDTO{MapperName: it.MapperName, Status: it.Status, TransactionId: it.TransactionId, ActionType: ActionType_Exec, OwnerId: it.OwnerId}, &result)
}
...
func (it *TransationRMSession) Begin() error {
	it.TransactionId = utils.CreateUUID()
	it.Status = Transaction_Status_Prepare
	var result TransactionRspDTO
	var err = it.Client.Call(TransactionReqDTO{MapperName: it.MapperName, Status: it.Status, TransactionId: it.TransactionId, ActionType: ActionType_Exec, OwnerId: it.OwnerId}, &result)
	return err
}
